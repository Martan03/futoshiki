...
\begin{itemize}
    ...
    \item \(R_2\) množina pravidel pro skoky ve tvaru
        ...
        \begin{itemize}
            \item Nechť \(a \in \Sigma\); \(u, v \in \Sigma^*\). Pak
            \(upayv \implies uaqyv\), právě když \(py \implies q \in R\).
        \end{itemize}
    \item \(s \in Q\) je počáteční stav
    \item \(F \subseteq Q\) je množina konečných stavů
\end{itemize}
...
\begin{tabular}{@{}p{4cm} p{10.5cm}@{}}
    \(Q_0\#\#R \implies Q_{01}\) & Posun doprava pro vynechání okraje
        mřížky \\
    \(Q_{01}\#\#D \implies Q_{11}\) & Posun dolů pro vynechání okraje mřížky \\

    & \\
    \(Q_{10}\#\#R \implies Q_{19}\) & Posun doprava a~přesun do stavu
        ověření konce mřížky \\
    \(Q_{11}FR \implies Q_{11}\) & Odstranění hodnoty z~domény řádku a
        sloupce a~posun doprava \\
    \(Q_{11}\#\#D \implies Q_{20}\) & Posun dolů pokud hranice mřížky \\
    \(Q_{11}R \implies Q_{11}\) & Jinak posun doprava \\
    \(Q_{19}\#D \implies Q_{11}\) & Následuje další řádek, přesun do stavu
        úpravy domén \\
    \(Q_{19}U \implies Q_{39}\) & Konec mřížky, přesun do stavu
        vytvoření domén polí \\

    & \\
    \(Q_{20}\#\#L \implies Q_{29}\) & Posun doleva a~přesun do stavu ověření
        konce mřížky \\
    \(Q_{21}FL \implies Q_{21}\) & Odstranění hodnoty z domény řádku a sloupce
        a~posun doleva \\
    \(Q_{21}\#\#D \implies Q_{10}\) & Posun dolů pokud hranice mřížky \\
    \(Q_{21}L \implies Q_{21}\) & Jinak posun doleva \\
    \(Q_{29}\#D \implies Q_{21}\) & Následuje další řádek, přesun do stavu
        úpravy domén \\
    \(Q_{29}U \implies Q_{49}\) & Konec mřížky, přesun do stavu
        vytvoření domén polí \\
\end{tabular}
...
\begin{tabular}{@{}p{4cm} p{10.5cm}@{}}
    \(Q_{30}\#\#R \implies Q_{39}\) & Posun doprava a přesun do stavu
        ověření konce mřížky \\
    \(Q_{31}ER \implies Q_{31}\) & Vytvoření domény na základě domén řádku
        a sloupce \\
    \(Q_{31}\#\#U \implies Q_{40}\) & Posun nahoru pokud hranice mřížky \\
    \(Q_{31}R \implies Q_{31}\) & Jinak posun doprava \\
    \(Q_{39}\#L \implies Q_{31}\) & Existuje předchozí řádek, přesun do stavu
        vytváření domén polí \\
    \(Q_{39} \implies Q_5\) & Konec mřížky \\

    & \\
    \(Q_{40}\#\#L \implies Q_{49}\) & Posun doleva a přesun do stavu
        ověření konce mřížky \\
    \(Q_{41}EL \implies Q_{41}\) & Vytvoření domény na základě domén řádku
        a sloupce \\
    \(Q_{41}\#\#U \implies Q_{30}\) & Posun nahoru pokud hranice mřížky \\
    \(Q_{41}L \implies Q_{41}\) & Jinak posun doleva \\
    \(Q_{49}\#U \implies Q_{41}\) & Existuje předchozí řádek, přesun do stavu
        vytváření domén polí \\
    \(Q_{49} \implies Q_5\) & Konec mřížky \\

    & \\
    \(Q_{5} \implies\) TODO & pravidla pro nerovnosti\\
\end{tabular}
...
\begin{tabular}{@{}p{4cm} p{10.5cm}@{}}
    \(Q_6\{L\} \implies Q_7\) & Skok na pole s nejmenší doménou \\
    & \\
\end{tabular}
