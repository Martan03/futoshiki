...
\begin{tabular}{@{}p{4cm} p{10.5cm}@{}}
    \(Q_0\#\#\ \rightarrow Q_{10}D\) & Posun dolů pro vynechání okraje
        mřížky \\

    & \\
    \(Q_{10}\#\# \rightarrow Q_{19}R\) & Posun doprava a přesun do stavu
        ověření konce mřížky \\
    \(Q_{11}\#\# \rightarrow Q_{12}R\) & Posun doprava pro vynechání okraje
        mřížky \\
    \(Q_{12}F \rightarrow Q_{12}R\) & Odstranění hodnoty z domény řádku a
        sloupce a posun doprava \\
    \(Q_{12}\#\# \rightarrow Q_{20}D\) & Posun dolů pokud hranice mřížky \\
    \(Q_{12} \rightarrow Q_{12}R\) & Jinak posun doprava \\
    \(Q_{19}\#\# \rightarrow Q_{39}U\) & Konec mřížky, přesun do stavu
        vytvoření domén polí \\
    \(Q_{19} \rightarrow Q_{11}L\) & Jinak posun zpět a úprava domén \\

    & \\
    \(Q_{20}\#\# \rightarrow Q_{29}L\) & Posun doleva a přesun do stavu
        ověření konce mřížky \\
    \(Q_{21}\#\# \rightarrow Q_{22}L\) & Posun doleva pro vynechání okraje
        mřížky \\
    \(Q_{22}F \rightarrow Q_{22}L\) & Odstranění hodnoty z domény řádku a
        sloupce a posun doleva \\
    \(Q_{22}\#\# \rightarrow Q_{10}D\) & Posun dolů pokud hranice mřížky \\
    \(Q_{22} \rightarrow Q_{22}L\) & Jinak posun doleva \\
    \(Q_{29}\#\# \rightarrow Q_{49}U\) & Konec mřížky, přesun do stavu
        vytvoření domén polí \\
    \(Q_{29} \rightarrow Q_{21}R\) & Jinak posun zpět a úprava domén \\

    & \\
    \(Q_{30}\#\# \rightarrow Q_{39}R\) & Posun doprava a přesun do stavu
        ověření konce mřížky \\
    \(Q_{31}\#\# \rightarrow Q_{32}R\) & Posun doprava pro vynechání okraje
        mřížky \\
    \(Q_{32}E \rightarrow Q_{32}R\) & Vytvoření domény na základě domén řádku
        a sloupce \\
    \(Q_{32}\#\# \rightarrow Q_{40}U\) & Posun nahoru pokud hranice mřížky \\
    \(Q_{32} \rightarrow Q_{32}R\) & Jinak posun doprava \\
    \(Q_{39}\#\# \rightarrow Q_5\) & Konec mřížky \\
    \(Q_{39} \rightarrow Q_{31}L\) & Jinak posun zpět a úprava domén \\

    & \\
    \(Q_{40}\#\# \rightarrow Q_{49}L\) & Posun doleva a přesun do stavu
        ověření konce mřížky \\
    \(Q_{41}\#\# \rightarrow Q_{42}L\) & Posun doleva pro vynechání okraje
        mřížky \\
    \(Q_{42}E \rightarrow Q_{42}L\) & Vytvoření domény na základě domén řádku
        a sloupce \\
    \(Q_{42}\#\# \rightarrow Q_{30}U\) & Posun nahoru pokud hranice mřížky \\
    \(Q_{42} \rightarrow Q_{42}L\) & Jinak posun doleva \\
    \(Q_{49}\#\# \rightarrow Q_5\) & Konec mřížky \\
    \(Q_{49} \rightarrow Q_{41}R\) & Jinak posun zpět a úprava domén \\

    & \\
    \(Q_{5} \rightarrow\) TODO & pravidla pro nerovnosti\\
\end{tabular}
...
\begin{itemize}
    \item \texttt{-s, --size [číslo]}: Velikost mřížky (výchozí je 4)
    \item \texttt{--solver [typ]}: Algoritmus pro řešení Futoshiki (více
        popsáno později)
    \item \texttt{-t, --theme [téma]}: Téma aplikace
    \begin{itemize}
...
\begin{itemize}
    \item \texttt{-s, --size [číslo]}: Velikost testované mřížky (lze zadat
        vícekrát)
    \item \texttt{--solver [typ]}: Testovaný algoritmus (lze zadat vícekrát)
    \item \texttt{-r, --repeats [číslo]}: Počet opakování testu
    \item \texttt{-b, --boards [číslo]}: Počet testovaných mřížek
\end{itemize}

\subsection*{Dostupné typy algoritmů}
\begin{itemize}
    \item \texttt{bt, backtract, backtracking}: Backtracking
...
vytvoří. Uživatel si v~něm může nastavit výchozí hodnoty pro velikost mřížky,
použitý algoritmus pro nalezení řešení a~téma aplikace.
...
V~tomto režimu může uživatel mřížku řešit manuálně.
...
by musel hru spouštět znovu za pomocí odpovídajících argumentů.
